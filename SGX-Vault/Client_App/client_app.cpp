...
static const uint8_t g_client_signature_private_key[32] = {
    0xef, 0x5c, 0x38, 0xb7, 0x6d, 0x4e, 0xed, 0xce, 0xde, 0x3b, 0x77,
    0x2d, 0x1b, 0x8d, 0xa7, 0xb9, 0xef, 0xdd, 0x60, 0xd1, 0x22, 0x50,
    0xcc, 0x90, 0xc3, 0xb5, 0x17, 0x54, 0xdc, 0x2f, 0xe5, 0x18};
...
std::string load_from_ini(std::string section, std::string key) {
  ...
  if (ret.length() == 0) {
    std::string message =
        "Failed to load setting " + key + " from settings_client.ini.";
    print_debug_message(message, ERROR);
    exit(1);
  }
...
void load_settings() {
  try {
    ...
    g_settings.min_isv_svn =
        std::stoi(load_from_ini("client", "MINIMUM_ISVSVN"));
    g_settings.req_isv_prod_id =
        std::stoi(load_from_ini("client", "REQUIRED_ISV_PROD_ID"));
    ...
    print_debug_message(
        "Invalid setting. Probably non-integer value was set illegally.",
        ERROR);
  ...
  uint32_t skip_flag =
      std::stoi(load_from_ini("client", "SKIP_MRENCLAVE_CHECK"));
...
int initialize_ra(std::string server_url, std::string &ra_ctx_b64,
  ...
  std::string client_id_b64 = std::string(base64_encode<char, char>(
      (char *)client_id_str.c_str(), client_id_str.length()));
  ...
  if (res->status == 200) {
    ...
    error_message = base64_decode<char, char>(
        (char *)res_json_obj["error_message"].ToString().c_str(),
        error_message_size);
...
int generate_kdk(EVP_PKEY *Gb, ra_session_t &ra_keys) {
  ...
  print_debug_binary("reversed shared secret Gab_x", Gab_x, secret_len,
                     DEBUG_LOG);
...
int process_session_keys(ra_session_t &ra_keys,
  ...
  print_debug_binary("reversed signature r", (uint8_t *)sigsp.x, 32, DEBUG_LOG);
  print_debug_binary("reversed signature s", (uint8_t *)sigsp.y, 32, DEBUG_LOG);
...
int get_quote(std::string server_url, std::string ra_ctx_b64,
  ...
  gb_x_b64 = std::string(base64_encode<char, uint8_t>(ra_keys.g_b, 32));
  gb_y_b64 = std::string(base64_encode<char, uint8_t>(&ra_keys.g_b[32], 32));

  sigsp_x_b64 =
      std::string(base64_encode<char, uint8_t>((uint8_t *)sigsp.x, 32));
  sigsp_y_b64 =
      std::string(base64_encode<char, uint8_t>((uint8_t *)sigsp.y, 32));
  ...
  if (res->status == 200) {
    // VKの生成
    aes_128bit_cmac(ra_keys.kdk, (uint8_t *)("\x01VK\x00\x80\x00"), 6,
                    ra_keys.vk);
    ...
    // Report
    // DataがGa、Gb、VKの連結に対するハッシュ値であるかをMAAに保証してもらう
...
int send_quote_to_maa(std::string quote_json, std::string &ra_report_jwt) {
  ...
  if (res == NULL) {
    std::string message =
        "Unknown error. Probably Attestation Provider is down.";
    print_debug_message(message, ERROR);
    exit(1);
  }
...
int verify_enclave(std::string ra_report_jwt, std::string quote_json,
  ...
  json::JSON jwt_obj = json::JSON::Load(std::string(
      base64url_decode<char, char>((char *)line.c_str(), jwt_payload_size)));
  ...
  print_debug_message(std::to_string(jwt_obj["x-ms-sgx-svn"].ToInt()),
                      DEBUG_LOG);
  print_debug_message("", DEBUG_LOG);
  ...
  print_debug_message(std::to_string(jwt_obj["x-ms-sgx-product-id"].ToInt()),
                      DEBUG_LOG);
  print_debug_message("", DEBUG_LOG);
  ...
  print_debug_binary("Derived hash of Ga||Gb||VK", data_hash, 32, DEBUG_LOG);
...
int send_ra_result(std::string server_url, std::string ra_ctx_b64,
                   bool ra_result) {
  print_debug_message("==============================================", INFO);
...
/* RAを実行する関数 */
int do_RA(std::string server_url, std::string &ra_ctx_b64, uint8_t *&sk,
          uint8_t *&mk) {
  print_debug_message("", INFO);
  print_debug_message("==============================================", INFO);
  ...
  /* セッション共通鍵SKとMKの生成 */
  aes_128bit_cmac(ra_keys.kdk, (uint8_t *)("\x01SK\x00\x80\x00"), 6,
                  ra_keys.sk);
  aes_128bit_cmac(ra_keys.kdk, (uint8_t *)("\x01MK\x00\x80\x00"), 6,
                  ra_keys.mk);
...
int aes_128_gcm_encrypt(uint8_t *plaintext, size_t p_len, uint8_t *key,
 ...
 * Enclaveとの暗号化通信を行うために利用可能 */
int aes_128_gcm_decrypt(uint8_t *ciphertext, size_t c_len, uint8_t *key,
...
int sample_remote_computation(std::string isv_url, std::string &ra_ctx_b64,
  ...
  /* SKで暗号化 */
  if (-1 == (aes_128_gcm_encrypt(plain_send_1, secret_1_len, sk, iv_send,
                                 cipher_send_1, tag_send_1))) {
    return -1;
  }

  if (-1 == (aes_128_gcm_encrypt(plain_send_2, secret_2_len, sk, iv_send,
                                 cipher_send_2, tag_send_2))) {
    return -1;
  }
  ...
  if (-1 == (aes_128_gcm_decrypt(cipher_result, cipher_result_len, mk,
                                 iv_result, tag_result, plain_result))) {
    return -1;
  }
